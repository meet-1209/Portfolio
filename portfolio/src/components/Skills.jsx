...
const Skills = () => {
  ...
  const technicalSkills = [
    ...
    { name: 'CSS', percentage: 100, icon: 'fab fa-css3-alt' },
  ...
  const professionalSkills = [
    ...
    { name: 'Communication', percentage: 75, icon: 'fas fa-comments' },
  ...
  const CircularProgress = ({ percentage, name, delay, icon }) => {
    const radius = 50
    ...
    return (
      <div
        className={`circular-skill ${isVisible ? 'animate-in' : ''}`}
      ...
      >
        ...
        <span className="skill-name">
          {icon && <i className={`${icon}`} style={{ marginRight: '8px', color: '#0ef' }}></i>}
          {name}
        </span>
      </div>
    )
          ...
          <div className="technical-skills">
            ...
            <div className="linear-skills">
              {technicalSkills.map((skill, index) => (
                <div
                  key={index}
                  className={`linear-skill ${isVisible ? 'animate-in' : ''}`}
                  style={{ transitionDelay: `${index * 0.15}s` }}
                >
                  <div className="skill-header">
                    <span className="skill-label">
                      {skill.icon && <i className={`${skill.icon}`} style={{ marginRight: '8px', color: '#0ef' }}></i>}
                      {skill.name}
                    </span>
